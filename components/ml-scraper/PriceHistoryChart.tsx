...
export function PriceHistoryChart({ history, productName }: Props) {
  ...
  return (
    <div className="border border-zinc-800 rounded-xl p-5 bg-zinc-900/40 mt-4">
      ...
      <p className="text-xs text-zinc-500 mb-5">Price history across {data.length} snapshots</p>

      {data.length < 2 ? (
        <p className="text-sm text-zinc-500 py-8 text-center">
          Only 1 snapshot available — history requires at least 2 sync runs.
        </p>
      ) : (
